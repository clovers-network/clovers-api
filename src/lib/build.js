  ...
  if (web3mode) {
    ...
    provider.getTransactionReceipt(tx).then((resp) => {
      ...
      resp.logs.map((log) => {
        ...
        console.log(logInfo)
        jsonRpcProvider.send('eth_getLogs', logInfo).then((result) => {
          console.log('jsonRpcProvider - eth_getLogs', result.length)
        })
        jsonRpcProvider.getLogs(logInfo).then((logs) => {
          console.log('jsonRpcProvider - getLogs', logs.length)
        })
  ...
  return new Promise((resolve, reject) => {
    ...
    if (key >= eventTypes.length) {
      ...
      if (web3mode) {
        ...
        jsonRpcProvider.send('eth_getLogs', {
          address: address.toLowerCase(),
          topics: eventType().topics,
          fromBlock: 1,
          toBlock: 'latest'
...
function processLogs () {
  ...
  return new Promise((resolve, reject) => {
    console.log('a')
    r.db('clovers_v2').table('logs').orderBy('blockNumber').run(db, (err, logs) => {
      console.log('b')
      if (err) return reject(err)
      processLog(logs).then(resolve).catch(reject)
      console.log('c')
    })
  })
}
...
function processLog (logs, i = 0) {
  ...
  return new Promise((resolve, reject) => {
    if (i >= logs.length) {
      ...
      let log = logs[i]
      handleEvent({log, db}).then(() => {
        processLog(logs, i + 1).then(resolve).catch(reject)
      }).catch(reject)
    }
  })
}
...
function nameClovers () {
  ...
  return new Promise ((resolve, reject) => {
    r.db('clovers').table('logs').filter({name: "newCloverName"}).orderBy('blockNumber').run(db, (err, logs) => {
      if (err) return reject(err)
      console.log('newCloverName:', logs.length)
      if (!logs.length) resolve()
      logs.toArray((err, result) => {
        if (err) return reject(err)
        nameClover(result).then(resolve).catch(reject)
      })
    })
  })
}
...
function nameClover (logs, key = 0) {
  return new Promise((resolve, reject) => {
    ...
    let log = logs[key]
    r.db('clovers_v2').table('clovers').get(log.data.board).run(db, (err, clover) => {
      if (err) return reject(err)
      if (!clover) {
        console.log('clover ' + log.data.board + ' not found')
        // return reject(new Error('clover ' + log.data.board + ' not found'))
        nameClover(logs, key + 1).then(resolve).catch(reject)
      } else {
        clover.name = xss(log.data.name)
        r.db('clovers_v2').table('clovers').get(log.data.board).update(clover).run(db, (err, result) => {
          if (err) return reject(err)
          nameClover(logs, key + 1).then(resolve).catch(reject)
        })
      }
...
function nameUsers () {
  ...
  return new Promise ((resolve, reject) => {
    r.db('clovers').table('logs').filter({name: "newUserName"}).orderBy('blockNumber').run(db, (err, logs) => {
      if (err) return reject(err)
      logs.toArray((err, result) => {
        if (err) return reject(err)
        nameUser(result).then(resolve).catch(reject)
      })
    })
  })
}
...
function nameUser (logs, key = 0) {
  return new Promise((resolve, reject) => {
    if (logs.length === key) resolve()
    let log = logs[key]
    r.db('clovers_v2').table('users').get(log.data.player).run(db, (err, user) => {
      if (err) return reject(err)
      if (!user) {
        console.log('user ' + log.data.player + ' not found')
        // return reject(new Error('user ' + log.data.player + ' not found'))
        nameUser(logs, key + 1).then(resolve).catch(reject)
      } else {
        user.name = xss(log.data.name)
        r.db('clovers_v2').table('users').get(log.data.player).update(user).run(db, (err, result) => {
          if (err) return reject(err)
          nameUser(logs, key + 1).then(resolve).catch(reject)
        })
      }
